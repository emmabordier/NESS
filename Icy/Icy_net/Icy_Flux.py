...
It depends on the iter_no and the final_error value you set, for the final_error value setted as 8.8e-6 and iter_no:1
It finishs at about 6hours~12hours by using the cuda gpu speed up
...
This a function to switch the label as (wavelength,Teff,logg,Rin,etc...) (normally we dont put
 wavelength as a training label/parameter because the training time is very long])

 It is for fature use
'''
...
def data_to_pytorch(filepath=None,ratio=95):
	...
	index_pos=np.where((Mass==2.0)&(tau!=4.0))[0]
...
#LR is learning rate
def train_function(training_x,training_y,final_error=0.0065,iter_no=1,LR=0.005,mini_batch=None,GPU=True):
	...
	optimizer=torch.optim.Adam(net.parameters(), lr=LR,weight_decay=0.1 ,betas=(0.9, 0.99))
	...
	file.write('finish at loop='+str(loop_count)+'\n')
	file.close()
...
text_name='Icy_Flux.txt'
#set the output and network parameter file name
net_name='Icy_Flux.pkl'
h5_name="Icy_Flux.h5"

#get the training and testing data
#Change filepath to path to grams_c.fits file
#training_x,training_y,testing_x,testing_y,x_max,x_min,wlen=data_to_pytorch(filepath=None,ratio=99);
training_x,training_y,testing_x,testing_y,x_max,x_min,wlen=data_to_pytorch(filepath='/Users/sundar/work/PRO/sundar_misc/GRAMS/',ratio=99);
net=Icy_Flux(training_x.numpy().shape[1],training_y.numpy().shape[1],None)
start_time=time.time()
#Change the error back to the original value
#train_function(training_x,training_y,final_error=0.007,iter_no=1,LR=0.005,mini_batch=None,GPU=False)
train_function(training_x,training_y,final_error=40.0,iter_no=1,LR=0.005,mini_batch=None,GPU=False)
net=net.cpu()
